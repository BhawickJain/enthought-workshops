...
def rank_order_neighbours(G):
    '''
    Takes a graph object G and returns an descending 
    list of number of neighbours each nodes has.
    ...
    N = len(G.nodes)
    s = pd.Series([len(list(G.neighbors(n))) for n in G.nodes])
    s = s.sort_values(ascending=False)
    
    return s

my_answer = rank_order_neighbours(G)
    
```

`[?]` What are valid inputs for the `pd.Series` and `pd.Dataframe` API?
...
def cust_deg_centrality(G):
    ...
    N = len(G.nodes)
    s = pd.Series([len(list(G.neighbors(n))) for n in G.nodes])
    s = s.sort_values(ascending=False)/(N-1)
    
    return s
                                        
cust_deg_centrality(G)
```
...
`[x]` Find out why the `cust_deg_centrality` function does not match the `nx.degree_centrality` result when the equation appears to be the same?
`[>]` They do match.
...
# x,y = ecdf_degree_centrality(G)

x,y = ecdf(pd.Series(nx.degree_centrality(G)))
plt.plot(x,y)
```
...
x,y = ecdf(rank_order_neighbours(G))
plt.plot(x,y)
```
...
N = len(G.nodes())
print("number of possible neighbours", N*(N-1))
```
...
a = ArcPlot(G, node_color="order", node_order="order")
a.draw()
plt.show()
```

```{python}
# ArcPlot appears broken in Binder instance as well of the tutorial
```
